...
fn main() {
    ...
    let adapter = pollster::block_on( // Waiting until this code is ready & run (async stuff b/c hardware)
    ...
    event_loop.run(move |event, _, control_flow| {
        *control_flow = event_loop::ControlFlow::Wait; // Why need to dereference here ?
        ...
        match event{
            ...
            Event::RedrawRequested(_) => { // Redrawing when any window gets a requested redraw I think??
                ...
                {//Inlaid scope makes it so encoder isn't moved (idk where the move happened either way but)
                    let _render_pass = encoder.begin_render_pass(
                        &wgpu::RenderPassDescriptor {
                            ...
                            color_attachments: &[Some(wgpu::RenderPassColorAttachment { //yeah absolutely no clue
                                view: &view,
                                resolve_target: None,
                                ops: wgpu::Operations {
                                    load: wgpu::LoadOp::Clear(wgpu::Color { r: 0.5, g: 0.05, b: blue_val, a: 1.0 }),
                                    store: true
                                }
                            })],
